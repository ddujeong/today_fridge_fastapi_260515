...
- 부족 재료가 여러 개면 완성 가능하다고 단정하지 않는다.
- 부족 재료가 있으면 "부족한 재료는 추가 확인이 필요합니다"라고 표현한다.
...
부족 재료: {missing}
사용자 조건: {conditions}
매칭률: {request.matchRate}
총점: {request.totalScore}
의미점수: {request.semanticScore}
하이브리드점수: {request.hybridScore}
